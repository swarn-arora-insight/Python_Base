...
def repair_athena_table(database, table_name, athena_output):
    ...
    print("🔄 Athena partition repair started")
...
def upload_df_to_s3_parquet(df: pd.DataFrame,bucket: str,project_name: str,database: str,table_name: str,athena_output: str):
    ...
    print("buffer",buffer)
    print(f"✅ Uploaded parquet to {s3_path}")
    ...
    try:
        ...
        print(f"⚠️ Athena repair failed (likely permission issue): {e}")
    ...
    try:
        ...
        print(":white_check_mark: S3 parquet file exists")
    except Exception as e:
        print(":x: S3 parquet file NOT found", e)
    return s3_path
