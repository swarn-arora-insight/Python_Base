...
class UserRepository:
    ...
    async def get_all_users(self):
        role_result = await self.db.execute(
            select(Role.role_id, Role.role_name).where(Role.is_active == 1)
        )
        role_list, org_list = {}, {}
            ...
            user_list.append(
                {
                    ...
                    "email": row.email,
                    "role": role_list.get(row.role_id, ""),
                    "org": org_list.get(row.org_id, ""),
                    "user_id": row.user_id,
                }
            )
        return user_list
        ...
        try:
            ...
            if user_instance:
                ...
                for key, value in user_details.items():
                    ...
                    )  # Update only the fields present in user_details
            else:
                # If the user does not exist, create a new user instance
                user_instance = User(
                    **user_details
                )  # Ensure user_details contains datetime objects
                self.db.add(user_instance)
    ...
    async def get_token_data(self, token: str) -> List:
        try:
            result = await self.db.execute(
                select(User.user_id).where(User.token == token, User.is_active == 1)
            )
    ...
    async def get_user_by_detail(self, payload: dict) -> List:
        try:
            if "email" in payload:
                email = payload["email"]
                result = await self.db.execute(
                    select(User.user_id).where(User.email == email, User.is_active == 1)
                )
                ...
                result = await self.db.execute(
                    select(User.user_id).where(
                        User.user_id == user_id, User.is_active == 1
                    )
                )
                user_data = [{"user_id": row.user_id} for row in result.all()]
                return user_data
    ...
    async def delete_token(self, token: str) -> tuple:
        try:
            token = token
            user = await self.db.scalar(select(User).where(User.token == token))
            if not user:
                return 404, "User not found"
    ...
    async def store_auth_key(self, user_id: int, auth_key: str) -> None:
        ...
        try:
            ...
            query = (
                update(User).where(User.user_id == user_id).values(auth_key=auth_key)
            )
    ...
    async def aes_encrypt(text: str) -> str:
        ...
        """
        cipher = Cipher(
            algorithms.AES(FRONTED_SECRET_KEY), modes.ECB(), backend=default_backend()
        )
